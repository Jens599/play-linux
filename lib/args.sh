...
play_ytdl_format_expression() {
  local format=$1 hardware=$2 video=${3:-bestvideo} codec=${4:-auto} height=${5:-from_quality} audio=${6:-bestaudio} fallback=${7:-best}
  local video_value audio_value fallback_value effective_height= effective_codec filters= fallback_filters= height_fallback=
  ...
  if [[ -n $effective_codec ]]; then
    ...
    printf '%s/%s%s%s\n' "$video_format" "$fallback_value" "$fallback_filters" "$height_fallback"
    return
  fi
